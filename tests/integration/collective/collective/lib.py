...
class UserLibrary(Library):
    ...
    def get_shared_library(self) -> str:
        from collective.install_info import libpath

        return os.path.join(
            libpath, f"libcollective{self.get_library_extension()}"
        )

    def get_c_header(self) -> str:
        from collective.install_info import header

        return header

    def get_registration_callback(self) -> str:
        return "collective_perform_registration"
...
user_lib = UserLibrary("collective")
