...
struct read_fn {
  ...
  void operator()(legate::Store &output, const std::string &filename,
                  int64_t my_id, int64_t num_readers) {
    using VAL = legate::legate_type_of<CODE>;

    int64_t code;
    size_t size;

    // All reader tasks need to read the header to figure out the total number
    // of elements
    ...
    // Then compute the extent for the output and create the output buffer to
    // populate
    int64_t my_ext = my_hi - my_lo;
    auto buf = output.create_output_buffer<VAL, 1>(legate::Point<1>(my_ext));

    // Skip to the right offset where the data assigned to this reader task
    // actually starts
    if (my_lo != 0)
    ...
    //
    // 1) Though this example binds the buffer after it's populated, it's not
    // strictly necessary.
    //    In fact, the create_output_buffer call takes an optional boolean
    //    argument that binds the created buffer immediately.
    // 2) The bind_data call takes the extents of data as an argument, just in
    // case the buffer
...
public:
  static void cpu_variant(legate::TaskContext &context) {
    ...
    // The task context contains metadata about the launch so each reader task
    // can figure out which part of the file it needs to read into the output.
    ...
    logger.print() << "Read " << filename << " (" << my_id + 1 << "/"
                   << num_readers << ")";

    legate::type_dispatch(output.code(), detail::read_fn{}, output, filename,
                          my_id, num_readers);
  }
};
