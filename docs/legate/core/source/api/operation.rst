...
parallelism from an operation by partitioning its store arguments. Since
operations rarely have the right semantics without any assumptions on how
arguments are partitioned, Legate provides APIs for developers to control how
stores are partitioned via `partitioning constraints`.
...
the copy turns into a full gather-scatter copy. Out-of-bounds indices in an
indirection can havoc the execution and the caller is responsible for making
sure the indices are within bounds.
...
``Fill`` is a special kind of operation for filling stores with constant values.
...
launch domain and those in the color space of a store partition is identity by
default and configured with a `projection function`, a Python function on
